...
function Activityhot_english({ match }: any) {
    useEffect(() => {
        document.title="Tran Dai Nghia High School for the Gifted Youth Union"   
    })
    const { id } = useParams();
    const linkValues = InitialVariable_english().activities.map(activity => activity.link.split('/')[1]);
    ...
    useLayoutEffect(() => {
        ...
        components.forEach((component) => {
            ...
            if (textTitle) {
                const nd = InitialVariable_english().activities[linkValues.indexOf(id as string)].title
            ...
            <div className='activityhot__data grid'>
                <div className="">
                    <h1 className="activityhot__title">
                        {InitialVariable_english().activities[linkValues.indexOf(id as string)].title}
                    </h1>
                    <p className="activityhot__description">
                        {docxText}
                    </p>
                    {/* <p className="activityhot__bq">
                        {InitialVariable_english().activities[linkValues.indexOf(id as string)].bq}
                    </p> */}

                    {/* {
                        InitialVariable_english().activities[linkValues.indexOf(id as string)].linkFB && (
                            <a href={InitialVariable_english().activities[linkValues.indexOf(id as string)].linkFB} target='_blank' className="activityhot__link">
                                Thông tin khác
                            </a>
                        )
                    } */}
...
export default Activityhot_english;
